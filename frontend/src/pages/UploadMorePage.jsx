...
export default function UploadMorePage() {
  ...
  // Fetch gallery ID from share token
  useEffect(() => {
    const fetchGallery = async () => {
      try {
        const gallery = await galleryAPI.getByToken(galleryToken);
  ...
  const handleSubmit = async () => {
    ...
    if (!galleryId) {
      setError('Gallery not loaded. Please wait a moment and try again.');
      return;
    }
    ...
    try {
      // Step 1: Upload photos without compression
      ...
      // Step 3: Navigate to gallery when complete
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-xl text-gray-600">
              Upload unlimited photos to your gallery. Large uploads are automatically processed in batches of 100.
            </p>
          </div>
          ...
          <div className="card text-center">
            <div {...getRootProps()} className={`relative border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-colors ${isDragActive ? 'border-primary-600 bg-primary-50' : 'border-gray-300 hover:border-primary-500'}`}>
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="font-bold text-gray-600">Drag & drop your event photos here</p>
            ...
            <div className="mt-8 text-center">
              ...
              >
                ...
                {loading ? `Uploading ${files.length} photos...` : `Upload ${files.length} Photo${files.length !== 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
